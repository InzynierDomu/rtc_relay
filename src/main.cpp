 ...
 * @return minutes
 */
int min_calculate(const DateTime& time)
{
  int time_minute = time.hour() * m_min_in_h;
...
{
  int current_minute = min_calculate(current);
  int start_minute = min_calculate(start);
  int end_minute = min_calculate(end);
